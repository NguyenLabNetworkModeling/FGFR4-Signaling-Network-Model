function [mex_output_s2] = sim_module_combo(sim,model)
...
target = sim.target;
dose = sim.dose;
mex_options.maxnumsteps = sim.maxsteps;
...
for ii = 1:length(target)
    ...
    if ismember(target{ii},{'FGFR4i_0','PI3Ki_0','ERBBi_0','AKTi_0','MEKi_0'})
        param_s2(ismember(param_names,target{ii})) = dose{ii};
    else
        % Drug treatment
        tmp_val = param_s2(ismember(param_names,target{ii}));
        param_s2(ismember(param_names,target{ii})) = tmp_val * (1/(1 + dose{ii}));
    end
end
