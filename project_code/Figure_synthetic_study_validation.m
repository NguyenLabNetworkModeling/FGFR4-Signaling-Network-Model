...
% Load synthetic data (with different noise level)
% e.g., Calibre_synthetic_data_50_percent.xlsx
clear; clc; close all;
load('synthetic_study.mat')
...
for ii = 1:length(calibr_var)
    ...
    shade_errorbar(xtime/60,calibr_avg(:,idx1),calibr_std(:,idx1),'Alpha',0.7,'Color',[0.00,0.45,0.74],'LineWidth',2)
    hold on
    shade_errorbar(xtime/60,measured_avg(:,idx2),measured_std(:,idx2)/sqrt(10),'Alpha',0.7,'Color',[0.85,0.33,0.10],'LineWidth',2)
...
for ii = 1:length(unmeasured_var)
    ...
    hold on
    shade_errorbar(xtime/60,unmeasured_avg(:,idx2),unmeasured_std(:,idx2)/sqrt(10),'Alpha',0.7,'Color',[0.85,0.33,0.10],'LineWidth',2.5)
